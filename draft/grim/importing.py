...
def module_from_path(module_path: str | Path, module_name: str | None = None) -> ModuleType:
    ...
    if module_path.is_file():
        ...
        if not init_file.is_file():
            raise FileNotFoundError(f"Directory '{module_path}' is not a valid package (missing __init__.py)")
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not create import spec for {module_name!r} at {module_path!s}")
