...
def orthogonize(text: str) -> str:
    ...
    match_results: dict[int, re.Match | None] = {line_no: field_type_value.match(line) for line_no, line in enumerate(lines)}
    matches: dict[int, dict[str, str]] = {line_no: match.groupdict(default="") for line_no, match in match_results.items() if match is not None}
    misses: dict[int, str] = {line_no: lines[line_no] for line_no, match in match_results.items() if match is None}

    margin_size: int = max((len(line) - len(line.lstrip())) for line in lines if line not in misses.values())
    ...
    for index in range(len(lines)):
        if index in matches:
            formatted_lines[index] = margin + "".join((formatted_fields[index], formatted_types[index], formatted_values[index]))
        else:
